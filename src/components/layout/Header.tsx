      ...
      {sessionUser?.isImpersonating && (
        <div className="bg-amber-500 text-slate-950 px-6 py-1.5 text-xs font-bold flex items-center justify-between shadow-md">
          ...
          <button
            ...
            className="bg-slate-950 text-white px-2.5 py-0.5 rounded-lg text-xs font-semibold hover:bg-slate-800 transition"
          >
            Exit Impersonation ➔
          </button>
        </div>
      ...
      <header className="h-16 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-950/80 backdrop-blur-md sticky top-0 z-20 flex items-center justify-between px-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-700 dark:text-slate-200">
            ...
            <span>{salonInfo?.name || sessionUser?.name || "Velvet Cut & Style"}</span>
          ...
          {sessionUser?.role === "SUPER_ADMIN" && (
            <button
              ...
              className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs font-bold hover:bg-amber-500/20 transition"
            >
          ...
          <div className="relative">
            <button
              ...
              className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-900 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-800 transition relative"
            >
              <Bell className="w-4 h-4" />
              {unreadCount > 0 && (
                <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-rose-500 ring-2 ring-white dark:ring-slate-950 animate-pulse"></span>
              )}
            </button>
              ...
              <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl p-4 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                ...
                <div className="space-y-2 max-h-72 overflow-y-auto">
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((n) => (
                      <div key={n.id} className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800 flex items-start gap-3">
                        {n.type === "booking" ? (
          ...
          <button
            ...
            title="Logout"
            className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-900 text-slate-600 dark:text-slate-300 hover:bg-red-500/10 hover:text-red-500 transition"
          >
